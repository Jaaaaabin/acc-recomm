...
class AuthoringBatchRunner:
    ...
    def _update_configuration(self, config_rvt_batch_dir=None, rvt_models_dir=None):
        ...
        rvt_models_dir = Path(rvt_models_dir)
        ...
        if Path(revit_file_list_path).exists():
            settings["revitFileListFilePath"] = str(Path(revit_file_list_path).resolve())
        
        if Path(task_script_path).exists():
            settings["taskScriptFilePath"] = str(Path(task_script_path).resolve())
